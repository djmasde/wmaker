...
wClientCheckProperty(WWindow *wwin, XPropertyEvent *event) 
{
	...
	if (wwin->frame && MyXFetchName(dpy, wwin->client_win, &tmp)) {
	    wFrameWindowChangeTitle(wwin->frame, tmp);
	...
	if (!wwin->icon) 
	    ...
	    MyXGetIconName(dpy, wwin->client_win, &new_title);
     ...
     case XA_WM_TRANSIENT_FOR:
	{
		...
		if (new_owner==None) {
			...
			wwin->window_flags.miniaturizable = 1;
			wwin->window_flags.miniaturize_button = 1;
			if (wwin->frame)
			    wWindowConfigureBorders(wwin);
		    }
		} else if (wwin->window_flags.miniaturizable) {
