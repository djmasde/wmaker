...
wRootMenuBindShortcuts(Window window)
{
	...
	if (ptr->modifier!=AnyModifier) {
	    XGrabKey(dpy, ptr->keycode, ptr->modifier|IGNORE_MOD_MASK,
	    ...
	    WMenuEntry *entry)
{
	...
	*k = 0;
    ...
    if (ksym==NoSymbol) {
	wwarning(_("%s:invalid kbd shortcut specification \"%s\""), file,
		 shortcutDefinition);
	free(ptr);
	return False;
    }
    ...
    if (ptr->keycode==0) {
	wwarning(_("%s:invalid key in shortcut \"%s\""), file,
		 shortcutDefinition);
	free(ptr);
	return False;
    }
    ...
    if (path[0][0]=='|') {
	    ...
	    path[i] = tmp;
            ...
            if (stat(path[i], &stat_buf)==0) {
                ...
                wwarning(_("%s:could not stat menu"), path[i]);
                /*goto finish;*/
            }
	     ...
	     char *params, char *file_name)
{
	...
	if (!params) {
	    ...
	    char *path;
            ...
            /* Using below form will not find files in DEF_CONFIG_PATH, and
             * return error for them. The above will anyway use wexpandpath()
             * to do ~ and $HOME expansion, so is better to use it. Dan.
             */
	    /*path = wexpandpath(params);*/
	    if (!path) {
		path = wstrdup(params);
	    }
...
readMenuFile(WScreen *scr, char *file_name)
{
	...
	if (strcasecmp(command, "MENU")==0) {
	    ...
	    wwarning(_("%s:invalid menu file"), file_name);
	    break;
	}
    }
...
configureMenu(WScreen *scr, proplist_t definition)
{
	...
	tmp = wstrdup(PLGetString(definition));
	...
	if (stat(path, &stat_buf)<0) {
	    wsyserror(_("%s:could not access menu \"%s\""), "defaults DB", path);
	    free(path);
	    free(tmp);
	    return NULL;
	}
	...
	if (PLIsArray(entry)) {
	    ...
	    addMenuEntry(menu, PLGetString(title), NULL, PLGetString(command),
