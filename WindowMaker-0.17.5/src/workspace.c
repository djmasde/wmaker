...
wWorkspaceForceChange(WScreen *scr, int workspace)
{
	...
	while (tmp) {
		...
		if (tmp->flags.selected) {
		    wWindowChangeWorkspace(tmp, workspace);
                    ...
                    if (!(tmp->flags.mapped || tmp->flags.hidden
     ...
     * 
     * But this completely breakes AutoArrangeIcons=NO. 
     */
    if (!wPreferences.sticky_icons)
        wArrangeIcons(scr);
...
onMenuEntryEdited(WMenu *menu, WMenuEntry *entry)
{
    ...
    entry->text = wstrdup(buf);
    
    /* update workspace */
    free(scr->workspaces[wspace]->name);
    scr->workspaces[wspace]->name = wstrdup(entry->text);
...
wWorkspaceMenuUpdate(WScreen *scr, WMenu *menu)
{
    ...
    /* this is only called when adding new workspaces, so is not very useful
     * because menu cn get bigger not smaller.
     */
