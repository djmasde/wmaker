...
allocGCs(WScreen *scr)
{
    ...
    gcv.background = scr->black_pixel;
    gcv.graphics_exposures = False;
    scr->clip_gc = XCreateGC(dpy, scr->w_win, GCForeground|GCBackground
			     |GCGraphicsExposures, &gcv);

    /* window title text GC */
    gcv.graphics_exposures = False;
    scr->window_title_gc = XCreateGC(dpy, scr->w_win, GCGraphicsExposures, 
				     &gcv);
...
wScreenInit(int screen_number)
{
    ...
    domain = wDefaultsInitDomain("WindowMaker", True);
    ...
    /* init other domains */
    scr->droot_menu = wDefaultsInitDomain("WMRootMenu", False);
    if (!scr->droot_menu->dictionary) {
	wwarning(_("could not read domain \"%s\" from defaults database"),
		 "WMRootMenu");
    }

    scr->dwindow = wDefaultsInitDomain("WMWindowAttributes", True);
    ...
    WMAddTimerHandler(3000, wDefaultsCheckDomains, scr);
