...
wAppIconCreateFromIcon(WIcon *icon)
{
    ...
    dicon->wm_class = wstrdup(icon->owner->wm_class);
    dicon->wm_instance = wstrdup(icon->owner->wm_instance);
...
WAppIcon*
wAppIconCreateForCommand(WScreen *scr, char *command, char *wm_instance,
			 char *wm_class)
{
    WAppIcon *dicon;
    ...
    dicon->icon = wIconCreateWithName(scr, wm_instance, wm_class);
#ifdef REDUCE_APPICONS
    dicon->num_apps = 0;
#endif
