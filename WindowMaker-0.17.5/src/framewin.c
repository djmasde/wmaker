...
wFrameWindowUpdateBorders(WFrameWindow *fwin, int flags)
{
	...
	if (flags & WFF_TITLEBAR) {
	    fwin->top_width = theight + ofs;

	    fwin->flags.need_texture_remake = 1;
	    
	    if (wPreferences.new_style) {
		if (fwin->left_button) {
		    wCoreConfigure(fwin->left_button, 0, ofs, bsize, bsize);
		}
		
		if (fwin->right_button) {
		    wCoreConfigure(fwin->right_button, width-bsize+1-ofs,
				   ofs, bsize, bsize);
		}
	    } else { /* !new_style */
		if (fwin->left_button) {
		...
		if (fwin->right_button) {
		    wCoreConfigure(fwin->right_button, width-bsize-3-2*ofs,
	    ...
	    for (i=0; i < (fwin->flags.menu?1:3); i++) {
	...
	if (flags & WFF_TITLEBAR) {		
	    ...
	    fwin->titlebar = wCoreCreate(fwin->core, ofs, ofs, width+1, 
					 theight);
	    
	    if (flags & WFF_LEFT_BUTTON) {
		fwin->flags.left_button = 1;
                if (wPreferences.new_style) {
                    fwin->left_button = wCoreCreate(fwin->core, ofs, ofs,
                                                    bsize, bsize);
		    if (width < theight*4) {
		...
		fwin->flags.right_button = 1;
                if (wPreferences.new_style) {
		    fwin->right_button =
			wCoreCreate(fwin->core, width-bsize+1-ofs, ofs,
				    bsize, bsize);
                } else {
		    fwin->right_button =
			wCoreCreate(fwin->titlebar, width-bsize-3-2*ofs,
				    (theight-bsize)/2, bsize, bsize);
...
updateTitlebar(WFrameWindow *fwin)
{
    ...
    x = 0;
    w = fwin->core->width - (fwin->flags.menu ? 2 : 0) + 1;
    ...
    wCoreConfigure(fwin->titlebar, x, fwin->flags.menu, w, theight);
}
...
wFrameWindowHideButton(WFrameWindow *fwin, int flags)
{
    ...
    if (wPreferences.new_style && fwin->titlebar) {
        updateTitlebar(fwin);
    }
}
...
wFrameWindowPaint(WFrameWindow *fwin)
{
	...
	w = MyTextWidth((*fwin->font)->font, title, titlelen);
	...
	switch (fwin->flags.justification) {
	 ...
	 default:
	    if (fwin->flags.hide_left_button || fwin->flags.hide_right_button)
	...
	XmbDrawString(dpy, fwin->titlebar->window, (*fwin->font)->font,
                      *fwin->title_gc, x, (*fwin->font)->y + 4, title,
		      titlelen);
#else /* !I18N_MB */
	XDrawString(dpy, fwin->titlebar->window, *fwin->title_gc, x,
                    (*fwin->font)->y + 4, title, titlelen);
#endif /* !I18N_MB */
...
wFrameWindowResize(WFrameWindow *fwin, int width, int height)
{
	...
	if (fwin->right_button) {
		...
		XMoveWindow(dpy, fwin->right_button->window, 
			    width - fwin->right_button->width + 1 - ofs, ofs);
        } else {
	    if (fwin->right_button)
		XMoveWindow(dpy, fwin->right_button->window, 
...
titlebarMouseDown(WObjDescriptor *desc, XEvent *event)
{
	...
	&& (event->xbutton.time - desc->click_timestamp <= DBLCLICK_TIME)) {
...
buttonMouseDown(WObjDescriptor *desc, XEvent *event)
{
    ...
    if (event->xbutton.time - desc->click_timestamp <= DBLCLICK_TIME) {
	desc->click_timestamp = -1;
