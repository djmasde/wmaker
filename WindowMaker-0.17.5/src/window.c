...
static void setupGrabs(WWindow *wwin);
static void removeGrabs(WWindow *wwin);
...
wWindowCreate()
{
    ...
    wwin->client_descriptor.click_timestamp = 0;
    return wwin;
}
...
setupGNUstepHints(WWindowAttributes *attribs, GNUstepWMAttributes *gs_hints)
{
	...
	attribs->titlebar = 
	    ((gs_hints->window_style & WMTitledWindowMask)?1:0);
	
	attribs->close_button = attribs->closable = 
	    ((gs_hints->window_style & WMClosableWindowMask)?1:0);
	
	attribs->miniaturize_button = attribs->miniaturizable =
	    ((gs_hints->window_style & WMMiniaturizableWindowMask)?1:0);
	
	attribs->resizebar = attribs->resizable = 
	    ((gs_hints->window_style & WMResizableWindowMask)?1:0);
    } else {
	/* setup the defaults */
	attribs->titlebar = 1;
	attribs->closable = 1;
	attribs->miniaturizable = 1;
	attribs->resizable = 1;
	attribs->close_button = 1;
	attribs->miniaturize_button = 1;
	attribs->resizebar = 1;
    }
    
    if (gs_hints->extra_flags & GSNoApplicationIconFlag) {
	attribs->application = 0;
    }
}
...
# ifdef DEBUG
	fprintf(stderr,"0x%08x: has decor hints [ ", (int)window);
# endif
	attribs->titlebar = 0;
	attribs->close_button = 0;
	attribs->miniaturize_button = 0;
	attribs->resizebar = 0;
...
# ifdef DEBUG
	fprintf(stderr,"0x%08x: has function hints [ ",(int)window);
# endif
	attribs->closable = 0;
	attribs->miniaturizable = 0;
	attribs->resizable = 0;
...
wManageWindow(WScreen *scr, Window window)
{
    ...
    if (!MyXFetchName(dpy, window, &title)) {
	title = NULL;
    }
	...
	if (wwin->transient_for==None || wwin->transient_for==window) {
	    ...
	    if (owner /*&& owner->main_window*/) {
		wwin->main_window = owner->main_window;
            } else {
		wwin->main_window = None;
            }
	    /* don't let transients be miniaturized if their owners
	     * are not */
    ...
    if (wwin->protocols.DELETE_WINDOW)
      ...
      wwin->window_flags.broken_close = 1;
...
#ifdef DEBUG
    	fprintf(stderr,"0x%08x: looking into motif hints\n",(int)window);
#endif
	...
	&& wwin->window_flags.miniaturizable) {
    ...
    if (win_state && !(wwin->wm_hints && wwin->wm_hints->flags&StateHint &&
        ...
        if (win_state->state->shaded>0 && wwin->window_flags.shadeable)
...
/*
    if (wPreferences.constrain_window_size) {
	if (width > wwin->screen_ptr->scr_width)
	    width = wwin->screen_ptr->scr_width;
	
	if (height > wwin->screen_ptr->scr_height)
	    height = wwin->screen_ptr->scr_height;
    }
*/
    ...
    if (wwin->window_flags.titlebar)
	foo |= WFF_TITLEBAR;
    if (wwin->window_flags.resizebar)
	foo |= WFF_RESIZEBAR;
    ...
    /* hide unused buttons */
    foo = 0;
    if (!wwin->window_flags.close_button)
	foo |= WFF_RIGHT_BUTTON;
    if (!wwin->window_flags.miniaturize_button)
	foo |= WFF_LEFT_BUTTON;
    if (foo!=0)
    ...
    wwin->frame->title = wstrdup(title ? title : DEF_WINDOW_TITLE);
    if (title)
	XFree(title);
		      ...
		      char *title, int x, int y, int width, int height)
{
    ...
    wwin->window_flags.titlebar = 1;
    wwin->flags.internal_window = 1;
    wwin->window_flags.focusable = 1;
    wwin->window_flags.bind_mouse = 1;
    wwin->window_flags.bind_keys = 1;
    ...
    wwin->frame->title = title ? wstrdup(title) : NULL;
    ...
    XLowerWindow(dpy, window);
    XMapSubwindows(dpy, wwin->frame->core->window);
    XMapRaised(dpy, wwin->frame->core->window);
    wwin->flags.mapped = 1;
    ...
    if (wPreferences.auto_focus) {
    	XGrabButton(dpy, AnyButton, MOD_MASK, wwin->client_win, 
...
wUnmanageWindow(WWindow *wwin, int restore)
{
    ...
    if (wwin->transient_for!=wwin->screen_ptr->root_win) {
	owner = wWindowFor(wwin->transient_for);
	if (owner) {
	...
	/* was the only window */
	wwin->screen_ptr->focused_window = NULL;
	...
	if (wwin->next)
	  wwin->next->prev = wwin->prev;
	    ...
	    wwin->screen_ptr->focused_window = wwin->prev;
	    wwin->screen_ptr->focused_window->next = NULL;
	}
	...
	if (wPreferences.focus_mode==WKF_CLICK) {
	    tmp = wWindowFor(wwin->transient_for);
	    if (tmp && (!tmp->flags.mapped || !tmp->window_flags.focusable)) {
		tmp = NULL;
	    }
	}
	...
	if (wPreferences.focus_mode==WKF_CLICK) {
	    wSetFocusTo(wwin->screen_ptr, tmp);
             ...
             */
            tmp = NULL;
            if (XQueryPointer(dpy, wwin->screen_ptr->root_win, &bar, &win,
...
{    
    ...
    UpdateSwitchMenu(wwin->screen_ptr, wwin, ACTION_CHANGE_FOCUS);
}
...
wWindowUnfocus(WWindow *wwin)
{
    ...
    if (wwin->window_flags.bind_mouse) {
	XUngrabButton(dpy, Button1, MOD_MASK, wwin->client_win);
    }
    if (wPreferences.focus_mode==WKF_CLICK)
	setupGrabs(wwin);
    UpdateSwitchMenu(wwin->screen_ptr, wwin, ACTION_CHANGE_FOCUS);
...
int req_width, req_height;	       /* new size of the client */
{
    ...
    /* sanity test. Remove this */
    if (!wwin->window_flags.closable && wwin->window_flags.close_button)
	puts("ERROR IN CLOSABLE");
    if (!wwin->window_flags.miniaturizable && wwin->window_flags.miniaturize_button)
	puts("ERROR IN MINIATU");
    /* end sanity test */
...
void wWindowSetShape(WWindow *wwin)
{
	...
	rect[count].x = 0;
	rect[count].y = 0;
	rect[count].width = wwin->frame->core->width;
	rect[count].height = wwin->frame->top_width;
	count++;
    }
    if (wwin->window_flags.resizebar) {
	rect[count].x = 0;
	...
	rect[count].width = wwin->frame->core->width;
	rect[count].height = wwin->frame->bottom_width;
	count++;
    }
...
wWindowSetKeyGrabs(WWindow *wwin)
{
	...
	if (key->modifier!=AnyModifier) {
	    XGrabKey(dpy, key->keycode, key->modifier|IGNORE_MOD_MASK,
...
static void
setupGrabs(WWindow *wwin)
{
    /* setup passive grabs for the client window */
...
static void
removeGrabs(WWindow *wwin)
{
    /* remove passive grabs from the client window frame */
...
resizebarMouseDown(WCoreWindow *sender, void *data, XEvent *event)
{
    ...
    event->xbutton.state &= ~IGNORE_MOD_MASK;
...
titlebarDblClick(WCoreWindow *sender, void *data, XEvent *event)
{
	...
	if (event->xbutton.state == 0) {
	    if (wwin->window_flags.shadeable) {
		/* shade window */
		if (wwin->flags.shaded)
		    wUnshadeWindow(wwin);
		else
	    ...
	    /* maximize window */
	    if (dir !=0 && wwin->window_flags.resizable) {
...
frameMouseDown(WObjDescriptor *desc, XEvent *event)
{
	...
	if (event->xbutton.button == Button3 && wwin->window_flags.resizable)
	    wMouseResizeWindow(wwin, event);
	else 
	    wMouseMoveWindow(wwin, event);
	XUngrabPointer(dpy, CurrentTime);
...
windowCloseClick(WCoreWindow *sender, void *data, XEvent *event)
{
    ...
    if (event->xbutton.state & ControlMask) {
	wClientKill(wwin);
    } else if (wwin->protocols.DELETE_WINDOW) {
	/* send delete message */
...
windowIconifyClick(WCoreWindow *sender, void *data, XEvent *event)
{
	...
	if ((event->xbutton.state & ControlMask) ||
	    ...
	    wapp = wApplicationOf(wwin->main_window);
	    if (wapp && wwin->window_flags.application)
