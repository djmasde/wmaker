...
showPosition(WWindow *wwin, int x, int y)
{
    ...
    sprintf(num, "%+i  %-+i", x, y);
    fw = MyTextWidth(scr->info_text_font->font, num, strlen(num));
#ifdef I18N_MB
    fh = scr->info_text_font->height;
    XmbDrawString(dpy, scr->geometry_display, scr->info_text_font->font, gc, 
		  (scr->geometry_display_width - 2 - fw) / 2,
		  (scr->geometry_display_height - 2 + fh) / 2,
		  num, strlen(num));
#else
    fh = scr->info_text_font->font->ascent;
    XDrawString(dpy, scr->geometry_display, gc,
		(scr->geometry_display_width - 2 - fw) / 2,
		(scr->geometry_display_height - 2 + fh) / 2,
		num, strlen(num));
#endif
...
showGeometry(WWindow *wwin, int x1, int y1, int x2, int y2, int direction)
{
    ...
    fw = MyTextWidth(scr->info_text_font->font, "8888", 4);
#ifdef I18N_MB
    fh = scr->info_text_font->height;
#else
    fh = scr->info_text_font->font->ascent;
	...
	/* Display the height. */
#ifdef I18N_MB
	XmbDrawString(dpy, root, scr->info_text_font->font, gc,
		      x - s + 3 - fw/2, my - fh/2 + scr->info_text_font->y + 1,
		      num, strlen(num));
#else
	XDrawString(dpy, root, gc, x - s + 3 - fw/2, my + fh/2, num,
		    strlen(num));
#endif
	XSetForeground(dpy, gc, scr->line_pixel);
	/* horizontal geometry */
...
#else
	XDrawString(dpy, root, gc, mx - fw/2 + 1, y - s + fh/2 + 1, num,
		    strlen(num));
#endif
	...
	/* Display the height. */
#ifdef I18N_MB
	XmbDrawString(dpy, scr->geometry_display, scr->info_text_font->font,
		      scr->info_text_gc,
		      (scr->geometry_display_width - 2 - fw) / 2,
		      (scr->geometry_display_height - 2 + fh) / 2,
		      num, strlen(num));
#else
	XDrawString(dpy, scr->geometry_display, scr->info_text_gc,
		    (scr->geometry_display_width - 2 - fw) / 2,
		    (scr->geometry_display_height - 2 + fh) / 2,
		    num, strlen(num));
#endif
...
drawTransparentFrame(WWindow *wwin, int x, int y, int width, int height)
{
    ...
    if (wwin->window_flags.resizebar && !wwin->flags.shaded) {
	...
	switch (event.type) {
	 ...
	 case MotionNotify:
		...
		if (!warped && !wPreferences.no_autowrap) {
		    ...
		    } else if (event.xmotion.x_root >= scr->scr_width - 1) {
...
wSelectWindows(WScreen *scr, XEvent *ev)
{
    ...
    while (1) {
	WMMaskEvent(dpy, ButtonReleaseMask | PointerMotionMask, &event);
