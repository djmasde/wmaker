...
WMPixmap* WMCreatePixmapFromXPMData(WMScreen *scrPtr, char **xpmData);
...
/*  RImage *image;*/
  ...
  scr = WMCreateSimpleApplicationScreen(dpy, NULL, &argc, argv);
  ...
  * not the name of the window, although it's not implemented yet */
  if (WMRunModalOpenPanelForDirectory(panel, initial, /*title*/ NULL, NULL) == True)
    printf("%s\n", WMGetFilePanelFile(panel));
  else
    printf("\n");
  return 0;
}
