...
static inline void StringCopy(char *, const char *);
...
Version(void)
{
  ...
  printf("Copyright (C) 1998 Alexander Kourakos\n");
}
...
Usage(void)
{
  ...
  printf("  -b, --bezier                   "
         "become a supercute bezier clock%s\n",
	 DO_BEZIER ? " (default)" : "");

  printf("  -n, --no-bezier                "
         "don't do all those fancy bezierishnesses%s\n",
	 DO_BEZIER ? "" : " (default)");
  ...
  printf("\n");
  printf("Added cool stuff from bezier clock:\n");
  printf("   Sitaram Iyer <ssiyer@cs.rice.edu>\n");
  printf("   Martin DeMello <mdemello@ruf.rice.edu>\n");
  printf("bclock-1.0 from http://dutian.twi.tudelft.nl/~vgeel/bclock/\n");
}
...
#define OPT_SECOND_HAND_LENGTH 0x104
  ...
  int opt_index = 0, o;
  static char *short_opts = "B:H:hS:svwbn";
  ...
  while ((o = getopt_long(ac, av, short_opts, long_opts, &opt_index)) != EOF) {
    ...
    case 'b':
      option.do_bezier = TRUE;
      break;

    case 'n':
      option.do_bezier = FALSE;
      break;
