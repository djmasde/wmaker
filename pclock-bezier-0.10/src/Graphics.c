...
UpdateClock(void)
{
  ...
  if (option.do_bezier || option.show_seconds) {
    ...
    if (option.do_bezier) {
      ...
      Transform trans = { 1, -SIZE/2, 1, -SIZE/2 };

      control_points[0].x = mx - (SIZE / 2);
      control_points[0].y = my - (SIZE / 2);
      control_points[1].x = sx - (SIZE / 2);
      control_points[1].y = sy - (SIZE / 2);
      control_points[2].x = 0.0;
      control_points[2].y = 0.0;
      control_points[3].x = hx - (SIZE / 2);
      control_points[3].y = hy - (SIZE / 2);

      b_points = bezier_points (4 * POINTS_PER_SEGMENT, control_points);
      if (b_points == NULL)
	return;

      x_points = TranslatePoints (b_points, 4 * POINTS_PER_SEGMENT, &trans, CoordModeOrigin);

      for (i = 0; i < 4 * POINTS_PER_SEGMENT; i++)
	x_points[i].x += SIZE, x_points[i].y += SIZE;

      XDrawLines (display, all_pm, gc, x_points, 4 * POINTS_PER_SEGMENT, CoordModeOrigin);
  ...
  *should_quit = FALSE;
  ...
  while (XPending(display)) {
    ...
    case Expose:
      ...
      break;
    case ButtonPress:
      option.do_bezier = !option.do_bezier;
      old_hour--; /* change something */
      UpdateClock();
      break;
    case DestroyNotify:
      *should_quit = TRUE;
